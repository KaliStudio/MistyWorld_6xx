            ...
            void OnGameObjectCreate(GameObject* gameObject)
            {
                switch (gameObject->GetEntry())
                {
                    ...
                    case GO_GIFT_OF_THE_OBSERVER_25:
                        GiftOfTheObserveurGUID = gameObject->GetGUID();
                        break;
                    default:
                        break;
                }
                ...
                switch (type)
                {
                    ...
                    case BOSS_ALGALON:
                        if (state == DONE)
                        {
                            ...
                            _algalonTimer = 61;
                            if (GameObject* gameObject = instance->GetGameObject(GiftOfTheObserveurGUID))
                                gameObject->SetRespawnTime(gameObject->GetRespawnDelay());
