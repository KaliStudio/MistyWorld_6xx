...
extern int main(int argc, char** argv)
{
...
#ifdef _WIN32 // Windows
    {
        ...
        if (highPriority)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                TC_LOG_INFO("serveur.authentification", "Niveau de priorité du processus d'authentification passe en niveau HAUT");
            else
                TC_LOG_ERROR("serveur.authentification", "Impossible de définir le  niveau de priorité du processus d'authentification.");
        }
    }
    ...
    if (affinity > 0)
    {
        ...
        if (sched_setaffinity(0, sizeof(mask), &mask))
			TC_LOG_ERROR("serveur.authentification", "Vous ne pouvez pas définir le processeurs utilisés (hex):%x, erreur:%s", affinity, strerror(errno));
        else
        {
            CPU_ZERO(&mask);
    ...
    if (highPriority)
    {
        if (setpriority(PRIO_PROCESS, 0, PROCESS_HIGH_PRIORITY))
            ...
            TC_LOG_INFO("serveur.authentification", "Niveau de priorité du processus d'authentification définie à %i", getpriority(PRIO_PROCESS, 0));
    }
