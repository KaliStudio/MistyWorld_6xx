...
{
    TC_LOG_INFO("serveur.authserveur", "Updating Realm List...");
    ...
    if (result)
    {
        do
        {
            ...
            if (init)
                TC_LOG_INFO("serveur.authserveur", "Added realm \"%s\" at %s:%u.", name.c_str(), m_realms[name].ExternalAddress.get_host_addr(), port);
        }
