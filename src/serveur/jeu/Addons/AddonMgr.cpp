...
void LoadFromDB()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u known addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
    else
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 known addons. DB table `addons` is empty!");
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u banned addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
