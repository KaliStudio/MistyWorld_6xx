...
uint32 GroupMgr::GenerateNewGroupDbStoreId()
{
    ...
    {
        TC_LOG_ERROR("misc", "Group storage ID overflow!! Can't continue, shutting down serveur. ");
...
uint32 GroupMgr::GenerateGroupId()
{
    ...
    {
        TC_LOG_ERROR("misc", "Group guid overflow!! Can't continue, shutting down serveur. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return NextGroupId++;
}
...
{
    {
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 group definitions. DB table `groups` is empty!");
            return;
        }
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    TC_LOG_INFO("serveur.loading", "Loading Group members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 group members. DB table `group_member` is empty!");
            return;
        }
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u group members in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    TC_LOG_INFO("serveur.loading", "Loading Group instance saves...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                   0           1        2              3             4             5            6
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 group-instance saves. DB table `group_instance` is empty!");
            return;
        }
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u group-instance saves in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
