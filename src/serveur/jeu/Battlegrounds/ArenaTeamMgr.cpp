...
uint32 ArenaTeamMgr::GenerateArenaTeamId()
{
    ...
    {
        TC_LOG_ERROR("bg.battleground", "Arena team ids overflow!! Can't continue, shutting down serveur. ");
...
void ArenaTeamMgr::LoadArenaTeams()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 arena teams. DB table `arena_team` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u arena teams in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
