...
void WorldSession::SendPacket(WorldPacket const* packet, bool forced /*= false*/)
{
    ...
    if (!forced)
    {
        OpcodeHandler const* handler = serveurOpcodeTable[packet->GetOpcode()];
            ...
            switch (opHandle->Status)
            {
                case STATUS_LOGGEDIN:
                    if (!_player)
                    {
                        ...
                        //! If player didn't log out a while ago, it means packets are being sent while the serveur does not recognize
