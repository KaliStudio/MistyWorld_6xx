...
void SmartWaypointMgr::LoadFromDB()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 SmartAI Waypoint Paths. DB table `waypoints` is empty.");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u SmartAI waypoint paths (total %u waypoints) in %u ms", count, total, GetMSTimeDiffToNow(oldMSTime));
...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 SmartAI scripts. DB table `smartai_scripts` is empty.");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u SmartAI scripts in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
