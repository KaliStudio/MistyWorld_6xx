...
void CreatureTextMgr::LoadCreatureTexts()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 ceature texts. DB table `creature_texts` is empty.");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u creature texts for %u creatures in %u ms", textCount, creatureCount, GetMSTimeDiffToNow(oldMSTime));
...
void CreatureTextMgr::LoadCreatureTextLocales()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u creature localized texts in %u ms", textCount, GetMSTimeDiffToNow(oldMSTime));
