...
void WardenCheckMgr::LoadWardenChecks()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 Warden checks. DB table `warden_checks` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u warden checks.", count);
}
...
void WardenCheckMgr::LoadWardenOverrides()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 Warden action overrides. DB table `warden_action` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u warden action overrides.", count);
}
