...
void LFGMgr::LoadRewards()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 lfg dungeon rewards. DB table `lfg_dungeon_rewards` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u lfg dungeon rewards in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 lfg entrance positions. DB table `lfg_entrances` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u lfg entrance positions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
