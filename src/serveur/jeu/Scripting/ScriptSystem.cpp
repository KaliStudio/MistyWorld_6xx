...
void SystemMgr::LoadScriptWaypoints()
{
    ...
    TC_LOG_INFO("serveur.loading", "Loading Script Waypoints for " UI64FMTD " creature(s)...", uiCreatureCount);
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 Script Waypoints. DB table `script_waypoint` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u Script Waypoint nodes in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
