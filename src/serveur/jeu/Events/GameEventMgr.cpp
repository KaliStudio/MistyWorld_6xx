...
{
    GameEventData &data = mGameEvent[event_id];
    bool serveurwide_evt = data.state != GAMEEVENT_NORMAL && data.state != GAMEEVENT_INTERNAL;

    RemoveActiveEvent(event_id);
    UnApplyEvent(event_id);

    if (overwrite && !serveurwide_evt)
    {
        data.start = time(NULL) - data.length * MINUTE;
        if (data.end <= data.start)
            data.end = data.start + data.length;
    }
    else if (serveurwide_evt)
    {
...
{
    {
        ...
        {
            mGameEvent.clear();
            TC_LOG_ERROR("serveur.loading", ">> Loaded 0 game events. DB table `game_event` is empty.");
            return;
        }
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));

    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Saves Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 game event saves in game events. DB table `game_event_save` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u game event saves in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));

        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Prerequisite Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 game event prerequisites in game events. DB table `game_event_prerequisite` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u game event prerequisites in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));

        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Creature Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 creatures in game events. DB table `game_event_creature` is empty");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u creatures in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));

        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event GO Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 gameobjects in game events. DB table `game_event_gameobject` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u gameobjects in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Model/Equipment Change Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 model/equipment changes in game events. DB table `game_event_model_equip` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u model/equipment changes in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 quests additions in game events. DB table `game_event_creature_quest` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u quests additions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event GO Quest Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 go quests additions in game events. DB table `game_event_gameobject_quest` is empty.");
        else
        {
            uint32 count = 0;
            do
    ...
    TC_LOG_INFO("serveur.loading", "Loading Game Event Quest Condition Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 quest event conditions in game events. DB table `game_event_quest_condition` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u quest event conditions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Condition Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 conditions in game events. DB table `game_event_condition` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u conditions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Condition Save Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 condition saves in game events. DB table `game_event_condition_save` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u condition saves in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event NPCflag Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 npcflags in game events. DB table `game_event_npcflag` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u npcflags in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Seasonal Quest Relations...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 seasonal quests additions in game events. DB table `game_event_seasonal_questrelation` is empty.");
        else
        {
            uint32 count = 0;
            do
    ...
    TC_LOG_INFO("serveur.loading", "Loading Game Event Vendor Additions Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 vendor additions in game events. DB table `game_event_npc_vendor` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u vendor additions in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Battleground Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 battleground holidays in game events. DB table `game_event_battleground_holiday` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u battleground holidays in game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Game Event Pool Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 pools for game events. DB table `game_event_pool` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u pools for game events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
}
