...
void LoadWeatherData()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 weather definitions. DB table `game_weather` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u weather definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
