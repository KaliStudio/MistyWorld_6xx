...
void TicketMgr::LoadTickets()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 GM tickets. DB table `gm_tickets` is empty!");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u GM tickets in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void TicketMgr::LoadSurveys()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded GM Survey count from database in %u ms", GetMSTimeDiffToNow(oldMSTime));
