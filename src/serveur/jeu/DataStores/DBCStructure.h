...
struct AchievementCriteriaEntry
{
    ...
    uint32  timedCriteriaStartType;                         // 12       m_timer_start_event Only appears with timed achievements, seems to be the type of starting a timed Achievement, only type 1 and some of type 6 need manual starting
                                                            //              1: ByEventId(?) (serveurside IDs),    2: ByQuestId,   5: ByCastSpellId(?)
...
struct CharStartOutfitEntry
{
    ...
    //int32 ItemDisplayId[MAX_OUTFIT_ITEMS];                // 29-52 not required at serveur side
    //int32 ItemInventorySlot[MAX_OUTFIT_ITEMS];            // 53-76 not required at serveur side
...
struct SpellCategoryEntry
{
    ...
    // char* Name;
    // uint32 unk  - Pandaria
    // uint32 unk2 - Pandaria - could be commun CD
};
