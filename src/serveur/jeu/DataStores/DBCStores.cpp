...
void LoadDBCStores(const std::string& dataPath)
{
    ...
    {
        TC_LOG_ERROR("misc", "Incorrect DataDir value in worldserveur.conf or ALL required *.dbc files (%d) not found by path: %sdbc", DBCFileCount, dataPath.c_str());
        exit(1);
    }
    ...
    if (!sAreaStore.LookupEntry(5491)          ||     // last area (areaflag) added in 6.0.1 (18443)
        !sCharTitlesStore.LookupEntry(389)     ||     // last char title added in 6.0.1 (18443)
        !sGemPropertiesStore.LookupEntry(2467) ||     // last gem property added in 6.0.1 (18443)
        !sMapStore.LookupEntry(1173)           ||     // last map added in 6.0.1 (18443)
        !sSpellStore.LookupEntry(163227)       )      // last spell added in 6.0.1 (18443)
    {
        TC_LOG_ERROR("misc", "You have _outdated_ DBC files. Please extract correct dbc files from client 6.0.1 18443.");
        exit(1);
    }

    TC_LOG_INFO("serveur.loading", ">> Initialized %d DBC data stores in %u ms", DBCFileCount, GetMSTimeDiffToNow(oldMSTime));
}
