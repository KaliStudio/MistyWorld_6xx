...
uint32 GuildMgr::GenerateGuildId()
{
    ...
    {
        TC_LOG_ERROR("guild", "Guild ids overflow!! Can't continue, shutting down serveur. ");
...
{
    // 1. Load all guilds
    TC_LOG_INFO("serveur.loading", "Loading guilds definitions...");
    {
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild definitions. DB table `guild` is empty.");
            return;
        }
        else
        {
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 2. Load all guild ranks
    TC_LOG_INFO("serveur.loading", "Loading guild ranks...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild ranks. DB table `guild_rank` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild ranks in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 3. Load all guild members
    TC_LOG_INFO("serveur.loading", "Loading guild members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild members. DB table `guild_member` is empty.");
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild members in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 4. Load all guild bank tab rights
    TC_LOG_INFO("serveur.loading", "Loading bank tab rights...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild bank tab rights. DB table `guild_bank_right` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u bank tab rights in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 5. Load all event logs
    TC_LOG_INFO("serveur.loading", "Loading guild event logs...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild event logs. DB table `guild_eventlog` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild event logs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 6. Load all bank event logs
    TC_LOG_INFO("serveur.loading", "Loading guild bank event logs...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild bank event logs. DB table `guild_bank_eventlog` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild bank event logs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 7. Load all news event logs
    TC_LOG_INFO("serveur.loading", "Loading Guild News...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild event logs. DB table `guild_newslog` is empty.");
        else
        {
            uint32 count = 0;
            do
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild new logs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 8. Load all guild bank tabs
    TC_LOG_INFO("serveur.loading", "Loading guild bank tabs...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild bank tabs. DB table `guild_bank_tab` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild bank tabs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
    ...
    TC_LOG_INFO("guild", "Filling bank tabs with items...");
    {
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild bank tab items. DB table `guild_bank_item` or `item_instance` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u guild bank tab items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
    ...
    TC_LOG_INFO("misc", "Validating data of loaded guilds...");
    {
        ...
        TC_LOG_INFO("serveur.loading", ">> Validated data of loaded guilds in %u ms", GetMSTimeDiffToNow(oldMSTime));
    }
}
...
void GuildMgr::LoadGuildXpForLevel()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 xp for guild level definitions. DB table `guild_xp_for_level` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_INFO("misc", "Unused (> Guild.MaxLevel in worldserveur.conf) level %u in `guild_xp_for_level` table, ignoring.", uint32(level));
            continue;
        }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u xp for guild level definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void GuildMgr::LoadGuildRewards()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 guild reward definitions. DB table `guild_rewards` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (strlen(achivements))
        {
            ...
            for (Tokenizer::const_iterator iter = tokenizer.begin(); iter != tokenizer.end(); iter++)
            {
                ...
                {
                    TC_LOG_ERROR("serveur.loading", "Guild rewards constains not existing achievement entry %u", achivementId);
                    continue;
                }
        ...
        {
            TC_LOG_ERROR("serveur.loading", "Guild rewards constains not existing item entry %u", reward.Entry);
            continue;
        }

        if (reward.Standing >= MAX_REPUTATION_RANK)
        {
            TC_LOG_ERROR("serveur.loading", "Guild rewards contains wrong reputation standing %u, max is %u", uint32(reward.Standing), MAX_REPUTATION_RANK - 1);
            continue;
        }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u guild reward definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
