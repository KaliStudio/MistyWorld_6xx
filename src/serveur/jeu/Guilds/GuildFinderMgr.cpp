...
{
    TC_LOG_INFO("serveur.loading", "Loading guild finder guild-related settings...");
    //                                                           0                1             2                  3             4           5             6         7
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild finder guild-related settings. Table `guild_finder_guild_settings` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u guild finder guild-related settings in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}

void GuildFinderMgr::LoadMembershipRequests()
{
    TC_LOG_INFO("serveur.loading", "Loading guild finder membership requests...");
    //                                                      0         1           2            3           4         5         6
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 guild finder membership requests. Table `guild_finder_applicant` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u guild finder membership requests in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
