...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recvData)
{
    ...
    // prevent deleting other players' characters using cheating outils
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    // for send serveur info and strings (config)
    ChatHandler chH = ChatHandler(pCurrChar->GetSession());

    // "GetAccountId() == db stored account id" checked in LoadFromDB (prevent login not own character using cheating outils)
    ...
    data.WriteBit(0);                   // serveur supports voice chat
    ...
    // Send MOTD
    {
        ...
        TC_LOG_DEBUG("network", "WORLD: Sent serveur info");
    }
