...
void WorldSession::HandleRepopRequestOpcode(WorldPacket& recvData)
{
    ...
    // so if the serveur is lagging enough the player can
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    ...
    // instant logout in taverns/cities or on taxi or for admins, gm's, mod's if its enabled in worldserveur.conf
    if (instantLogout)
    {
        LogoutPlayer(true);
        return;
...
void WorldSession::HandleLogoutCancelOpcode(WorldPacket& /*recvData*/)
{
    ...
    // Player have already logged out serveurside, too late to cancel
    if (!GetPlayer())
        return;
...
{
    // Instructs the serveur we wish to receive few amounts of large packets (SMSG_MULTIPLE_PACKETS?)
