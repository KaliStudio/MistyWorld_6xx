...
{
    // Collect commun pointers into a set to avoid deleting the same memory more than once
...
void TransportMgr::LoadTransportTemplates()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 transport templates. DB table `gameobject_template` has no transports!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u transport templates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void TransportMgr::SpawnContinentTransports()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Spawned %u continent transports in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
