...
bool Creature::Create(uint32 guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, uint32 team, float x, float y, float z, float ang, const CreatureData* data)
{
    ...
    {
        m_serveurSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GHOST, GHOST_VISIBILITY_GHOST);
        m_serveurSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GHOST, GHOST_VISIBILITY_GHOST);
    }
...
void Creature::FocusTarget(Spell const* focusSpell, WorldObject const* target)
{
    ...
    // Set serveurside orientation if needed (needs to be after attribute check)
    SetInFront(target);
}
