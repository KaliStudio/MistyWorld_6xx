...
enum UnitFlags
{
    UNIT_FLAG_SERVER_CONTROLLED     = 0x00000001,           // set only when unit movement is controlled by serveur - by SPLINE/MONSTER_MOVE packets, together with UNIT_FLAG_STUNNED; only set to units controlled by client; client function CGUnit_C::IsClientControlled returns false when set for owner
...
enum MovementFlags
{
    ...
    // Movement flags allowed for creature in CreateObject - we need to keep all other enabled serveurside
...
class Unit : public WorldObject
{
    public:
        ...
        SharedVisionList const& GetSharedVisionList() { return m_communVision; }
        void AddPlayerToVision(Player* player);
        void RemovePlayerFromVision(Player* player);
        bool HasSharedVision() const { return !m_communVision.empty(); }
    ...
    protected:
        ...
        SharedVisionList m_communVision;
