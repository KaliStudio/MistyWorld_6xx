...
#pragma warning(default:4355)
    ...
    m_serveurSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GHOST, GHOST_VISIBILITY_ALIVE);
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    // dead loop is killing the serveur probably
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    //! Probably used for debugging purposes, as it is not known to appear on retail serveurs
...
{
    if (m_communVision.empty())
    {
        setActive(true);
        SetWorldObject(true);
    }
    m_communVision.push_back(player);
}

// only called in Player::SetSeer
void Unit::RemovePlayerFromVision(Player* player)
{
    m_communVision.remove(player);
    if (m_communVision.empty())
    {
        setActive(false);
...
{
    return (m_serveurSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GM) > SEC_PLAYER) ? false : true;
}

void Unit::SetVisible(bool x)
{
    if (!x)
        m_serveurSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_GAMEMASTER);
    else
        m_serveurSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
...
void Unit::SendTeleportPacket(Position& pos)
{
    ...
    // SMSG_MOVE_TELEPORT is sent to self in order to trigger CMSG_MOVE_TELEPORT_ACK and update the position serveur side
...
//! Only serveur-side orientation update, does not broadcast to client
...
//! Only serveur-side height update, does not broadcast to client
...
void Unit::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    for (uint16 index = 0; index < valCount; ++index)
    {
            ...
            (index == UNIT_FIELD_AURA_STATE && HasFlag(UNIT_FIELD_AURA_STATE, PER_CASTER_AURA_STATE_MASK))))
        {
            ...
            // FIXME: Some values at serveur stored in float format but must be sent to client in uint32 format
