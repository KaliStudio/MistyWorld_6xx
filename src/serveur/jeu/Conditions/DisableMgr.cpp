...
void LoadDisables()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 disables. DB table `disables` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u disables in %u ms", total_count, GetMSTimeDiffToNow(oldMSTime));
}
...
void CheckQuestDisables()
{
    ...
    if (!count)
    {
        TC_LOG_INFO("serveur.loading", ">> Checked 0 quest disables.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Checked %u quest disables in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
