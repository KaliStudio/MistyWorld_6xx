...
void ConditionMgr::LoadConditions(bool isReload)
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 conditions. DB table `conditions` is empty!");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
bool ConditionMgr::addToSpellImplicitTargetConditions(Condition* cond)
{
    ...
    ASSERT(spellInfo);
    std::list<uint32> communMasks;
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        // check if effect is already a part of some commun mask
        bool found = false;
        for (std::list<uint32>::iterator itr = communMasks.begin(); itr != communMasks.end(); ++itr)
        {
            if ((1<<i) & *itr)
            {
                found = true;
        ...
        // build new commun mask with found effect
        uint32 communMask = (1<<i);
        ...
        for (uint8 effIndex = i+1; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
        {
            if (spellInfo->Effects[effIndex].ImplicitTargetConditions == cmp)
                communMask |= 1<<effIndex;
        }
        communMasks.push_back(communMask);
    }
    ...
    for (std::list<uint32>::iterator itr = communMasks.begin(); itr != communMasks.end(); ++itr)
    {
        ...
        if (uint32 commonMask = *itr & conditionEffMask)
        {
            ...
            // get commun data
            ConditionList* communList = spellInfo->Effects[firstEffIndex].ImplicitTargetConditions;

            // there's already data entry for that communMask
            if (communList)
            {
            ...
            // no data for commun mask, we can create new submask
            else
            {
                // add new list, create new commun mask
                communList = new ConditionList();
                bool assigned = false;
                for (uint8 i = firstEffIndex; i < MAX_SPELL_EFFECTS; ++i)
                {
                    if ((1<<i) & commonMask)
                    {
                        spellInfo->Effects[i].ImplicitTargetConditions = communList;
                        assigned = true;
                    }
                }
