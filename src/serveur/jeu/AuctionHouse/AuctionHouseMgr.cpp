...
void AuctionHouseMgr::LoadAuctionItems()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 auction items. DB table `auctionhouse` or `item_instance` is empty!");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u auction items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void AuctionHouseMgr::LoadAuctions()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 auctions. DB table `auctionhouse` is empty.");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u auctions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
{
    // Deletes expired auctions. Should be called at serveur start before loading auctions.
    ...
    if (!expAuctions)
    {
        TC_LOG_INFO("serveur.loading", ">> No expired auctions to delete");
    ...
    TC_LOG_INFO("serveur.loading", ">> Deleted %u expired auctions in %u ms", expirecount, GetMSTimeDiffToNow(oldMSTime));
