...
void PoolMgr::LoadFromDB()
{
    // Pool templates
    {
        ...
        {
            mPoolTemplate.clear();
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 object pools. DB table `pool_template` is empty.");
            return;
        }
        ...
        TC_LOG_INFO("serveur.loading", ">> Loaded %u objects pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    // Creatures

    TC_LOG_INFO("serveur.loading", "Loading Creatures Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 creatures in  pools. DB table `pool_creature` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u creatures in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Gameobjects

    TC_LOG_INFO("serveur.loading", "Loading Gameobject Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 gameobjects in  pools. DB table `pool_gameobject` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u gameobject in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Pool of pools

    TC_LOG_INFO("serveur.loading", "Loading Mother Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 pools in pools");
        }
        else
        {
            uint32 count = 0;
                ...
                for (SearchMap::iterator poolItr = mPoolSearchMap.find(i); poolItr != mPoolSearchMap.end(); poolItr = mPoolSearchMap.find(poolItr->second))
                {
                    ...
                    if (checkedPools.find(poolItr->second) != checkedPools.end())
                    {
                        ...
                        for (std::set<uint32>::const_iterator itr=checkedPools.begin(); itr != checkedPools.end(); ++itr)
                            ss << *itr << ' ';
                        ss << "create(s) a circular reference, which can cause the serveur to freeze.\nRemoving the last link between mother pool "
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u pools in mother pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    TC_LOG_INFO("serveur.loading", "Loading Quest Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Loaded 0 quests in pools");
        }
        else
        {
            ...
            TC_LOG_INFO("serveur.loading", ">> Loaded %u quests in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // The initialize method will spawn all pools not in an event and not in another pool, this is why there is 2 left joins with 2 null checks
    TC_LOG_INFO("serveur.loading", "Starting objects pooling system...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            TC_LOG_INFO("serveur.loading", ">> Pool handling system initialized, 0 pools spawned.");
        }
        else
        {
            uint32 count = 0;
