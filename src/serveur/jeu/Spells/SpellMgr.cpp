...
void SpellMgr::LoadSpellRanks()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell rank records. DB table `spell_ranks` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell rank records in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellRequired()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell required records. DB table `spell_required` is empty.");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell required records in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
...
void SpellMgr::LoadSpellLearnSkills()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u Spell Learn Skills from DBC in %u ms", dbc_count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellLearnSpells()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell learn spells. DB table `spell_learn_spell` is empty.");
        return;
    }
    ...
    */
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell learn spells, %u found in Spell.dbc and %u from TalentTab.dbc in %u ms", count, dbc_count, 0, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell target coordinates. DB table `spell_target_position` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell teleport coordinates in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellGroups()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell group definitions. DB table `spell_group` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellGroupStackRules()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell group stack rules. DB table `spell_group_stack_rules` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell group stack rules in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell proc event conditions. DB table `spell_proc_event` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u extra spell proc event conditions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellProcs()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell proc conditions and data. DB table `spell_proc` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell proc conditions and data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellBonusess()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell bonus data. DB table `spell_bonus_data` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u extra spell bonus data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellThreats()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 aggro generating spells. DB table `spell_threat` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u SpellThreatEntries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSkillLineAbilityMap()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u SkillLineAbility MultiMap Data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell pet auras. DB table `spell_pet_auras` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell pet auras in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadEnchantCustomAttr()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u custom enchant attributes in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellEnchantProcData()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell enchant proc event conditions. DB table `spell_enchant_proc_data` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u enchant proc data definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellLinked()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 linked spells. DB table `spell_linked_spell` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u linked spells in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadPetLevelupSpellMap()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u pet levelup and default spells for %u families in %u ms", count, family_count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadPetDefaultSpells()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded addition spells for %u pet spell data entries in %u ms", countData, GetMSTimeDiffToNow(oldMSTime));

    TC_LOG_INFO("serveur.loading", "Loading summonable creature templates...");
    oldMSTime = getMSTime();
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u summonable creature templates in %u ms", countCreature, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellAreas()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 spell area requirements. DB table `spell_area` is empty.");
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u spell area requirements in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellInfoStore()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded SpellInfo store in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellInfoCustomAttributes()
{
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded SpellInfo custom attributes in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
            ...
            case 64486: // Empowering Shadows (Yogg-Saron)
                ...
                break;
            case 62301: // Cosmic Smash (Algalon the Observeur)
                spellInfo->MaxAffectedTargets = 1;
                break;
            case 64598: // Cosmic Smash (Algalon the Observeur)
                spellInfo->MaxAffectedTargets = 3;
                break;
            case 62293: // Cosmic Smash (Algalon the Observeur)
                spellInfo->Effects[EFFECT_0].TargetB = SpellImplicitTargetInfo(TARGET_DEST_CASTER);
                break;
            case 62311: // Cosmic Smash (Algalon the Observeur)
            case 64596: // Cosmic Smash (Algalon the Observeur)
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded SpellInfo corrections in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
