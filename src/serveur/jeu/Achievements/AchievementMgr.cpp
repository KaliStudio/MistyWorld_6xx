...
void AchievementMgr<Player>::LoadFromDB(PreparedQueryResult achievementResult, PreparedQueryResult criteriaResult)
{
    if (achievementResult)
    {
        do
        {
            ...
            // must not happen: cleanup at serveur startup in sAchievementMgr->LoadCompletedAchievements()
...
void AchievementGlobalMgr::LoadAchievementCriteriaList()
{
    ...
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 achievement criteria.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u achievement criteria and %u guild achievement crieteria in %u ms", criterias, guildCriterias, GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadAchievementReferenceList()
{
    ...
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 achievement references.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u achievement references in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadAchievementCriteriaData()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 additional achievement criteria data. DB table `achievement_criteria_data` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u additional achievement criteria data in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadCompletedAchievements()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 realm first completed achievements. DB table `character_achievement` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %lu realm first completed achievements in %u ms", (unsigned long)m_allCompletedAchievements.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadRewards()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR("serveur.loading", ">> Loaded 0 achievement rewards. DB table `achievement_reward` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %u achievement rewards in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void AchievementGlobalMgr::LoadRewardLocales()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("serveur.loading", ">> Loaded 0 achievement reward locale strings.  DB table `locales_achievement_reward` is empty");
        return;
    }
    ...
    TC_LOG_INFO("serveur.loading", ">> Loaded %lu achievement reward locale strings in %u ms", (unsigned long)m_achievementRewardLocales.size(), GetMSTimeDiffToNow(oldMSTime));
}
