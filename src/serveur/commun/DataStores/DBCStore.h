        ...
        bool Load(char const* fn, SqlDbc* sql)
        {
            ...
            if (sql)
            {
                ...
                if (result)
                {
                    ...
                    {
                        TC_LOG_ERROR("serveur.loading", "Invalid index pos for dbc:'%s'", sql->sqlTableName.c_str());
                        return false;
                    }
                }
            }
                ...
                if (indexTable.asT)
                {
                    ...
                    do
                    {
                        ...
                        if (sql->indexPos >= 0)
                        {
                            ...
                            {
                                TC_LOG_ERROR("serveur.loading", "Index %d already exists in dbc:'%s'", id, sql->sqlTableName.c_str());
                                return false;
                            }
                        ...
                        for (; columnNumber < sql->formatString->size(); ++columnNumber)
                        {
                                ...
                                switch (fmt[columnNumber])
                                {
                                    ...
                                    case FT_BYTE:
                                        *reinterpret_cast<uint8*>(&sqlDataTable[offset]) = fields[sqlColumnNumber].GetUInt8();
                                        ...
                                        break;
                                    case FT_STRING:
                                        TC_LOG_ERROR("serveur.loading", "Unsupported data type in table '%s' at char %d", sql->sqlTableName.c_str(), columnNumber);
                                        return false;
                                    case FT_SORT:
                                        break;
                                    default:
                            ...
                            {
                                TC_LOG_ERROR("serveur.loading", "Incorrect sql format string '%s' at char %d", sql->sqlTableName.c_str(), columnNumber);
                                return false;
                            }
                        }
                        ...
                        {
                            TC_LOG_ERROR("serveur.loading", "SQL and DBC format strings are not matching for table: '%s'", sql->sqlTableName.c_str());
                            return false;
                        }
