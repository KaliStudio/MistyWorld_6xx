...
class DatabaseWorkerPool
{
    public:
        ...
        bool Open(const std::string& infoString, uint8 async_threads, uint8 synch_threads)
        {
            ...
            for (uint8 i = 0; i < async_threads; ++i)
            {
                ...
                if (res) // only check mysql version if connection is valid
                    WPFatal(mysql_get_serveur_version(t->GetHandle()) >= MIN_MYSQL_SERVER_VERSION, "TrinityCore does not support MySQL versions below 5.1");
        ...
        */
        ...
        //! Keeps all our MySQL connections alive, prevent the serveur from disconnecting us.
        void KeepAlive()
        {
    ...
    private:
        ...
        ACE_Activation_Queue*           _queue;             //! Queue commun by async worker threads.
