...
void RARunnable::run()
{
    ...
    {
        TC_LOG_ERROR("serveur.Mondial", "Trinity RA can not bind to port %d on %s", raPort, stringIp.c_str());
        return;
    }

    TC_LOG_INFO("serveur.Mondial", "Starting Trinity RA on port %d on %s", raPort, stringIp.c_str());
    ...
    TC_LOG_DEBUG("serveur.Mondial", "Trinity RA thread exiting");
}
