/**
 ...
 with raw UDP/TCP (e.g. connecting to an HTTP serveur). 
...
/**
 ...
 on the serveur using NetListener::waitForConnection.  Set the reference
 ...
 <OL>
   <LI> Create a G3D::NetworkDevice (if you are using G3D::GApp, it creates 
        one for you) on the client and on the serveur.
   <LI> On the serveur, create a G3D::NetListener using 
        G3D::NetworkDevice::createListener
   <LI> On the serveur, invoke G3D::NetListener::waitForConnection.
   <LI> On the client, call G3D::NetworkDevice::createReliableConduit.  
        You will need the serveur's G3D::NetAddress.  Consider using
...
public:
    ...
    /**
     Client invokes this to connect to a serveur.  The call blocks until the 
    ...
    /** Send the same message to a number of conduits.  Useful for sending
        data from a serveur to many clients (only serializes once). */
    template<typename T>
...
broadcast to all serveurs on a LAN, set the third optional argument to 
true (the default is false for no broadcast).  You can also set up the
receive port as if it was a serveur to send and receive from a single 
LightweightConduit.
...
public:
    ...
    /** Send the same message to multiple addresses (only serializes once).
        Useful when serveur needs to send to a known list of addresses
...
/**
 Runs on the serveur listening for clients trying to make reliable connections.
 */
