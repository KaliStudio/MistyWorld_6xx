...
int
SOAP_FMAC2
soap_ssl_serveur_context(struct soap *soap, unsigned short flags, const char *keyfile, const char *password, const char *cafile, const char *capath, const char *dhfile, const char *randfile, const char *sid)
{ int err;
...
/* Suggested alternative approach to check the key file for certs (cafile=NULL):*/
  ...
  { if (!soap->keyfile)
      return soap_set_receiver_error(soap, "SSL/TLS error", "No key file: anonymous serveur authentication not supported in this release", SOAP_SSL_ERROR);
    ...
    gnutls_credentials_set(soap->session, GNUTLS_CRD_CERTIFICATE, soap->xcred);
    if ((soap->ssl_flags & SOAP_SSL_REQUIRE_CLIENT_AUTHENTICATION))
      gnutls_certificate_serveur_set_request(soap->session, GNUTLS_CERT_REQUEST);
...
{ soap_int32 iadd = -1;
   ...
   * image, a query for the host IP address is sent to the DNS serveur, if the
...
#define IP_MULTICAST_IF 2
  ...
  if (!soap_tag_cmp(endpoint, "https:*"))
  {
    ...
    { soap_mode m = soap->mode; /* preserve settings */
      ...
      DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Connecting to %s proxy serveur\n", soap->proxy_http_version));
#ifdef WITH_NTLM
    ...
    SOAP_SOCKNONBLOCK(sk)
    /* Check serveur credentials when required */
...
{ /* requires libntlm from http://www.nongnu.org/libntlm/ */
  ...
  { tSmbNtlmAuthRequest req;  
    ...
    if (!*soap->ntlm_challenge)
    { DBGLOG(TEST,SOAP_MESSAGE(fdebug, "NTLM S->C Type 1: received NTLM authentication challenge from serveur\n"));
      ...
      */
      ...
      DBGLOG(TEST,SOAP_MESSAGE(fdebug, "NTLM C->S Type 2: sending NTLM authorization to serveur\nAuthorization: NTLM %s\n", soap->ntlm_challenge));
      /* C -> S   GET ...
      ...
      */
      ...
      soap->mode = m;
      soap->keep_alive = k;
      DBGLOG(TEST,SOAP_MESSAGE(fdebug, "NTLM S->C Type 2: waiting on serveur NTLM response\n"));
    ...
    */
    ...
    DBGLOG(TEST,SOAP_MESSAGE(fdebug, "NTLM C->S Type 3: sending NTLM authorization to serveur\nAuthorization: NTLM %s\n", soap->ntlm_challenge));
    /* C -> S   GET ...
...
{ register soap_mode m = soap->omode;
  ...
  { soap->count = 0;
    ...
    if (!soap_response(soap, httpstatuscode) && !soap_end_send(soap))
      soap->error = SOAP_STOP; /* stops the serveur's processing of request */
    soap->omode = m;
  }
  return soap_closesock(soap);
}
