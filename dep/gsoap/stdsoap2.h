...
/* Max iterations in soap_serve() to keep serveur connection alive */
...
/* Exceptional gSOAP HTTP serveur response status codes >= 1000 */
...
/* SSL client/serveur authentication settings */

#define SOAP_SSL_NO_AUTHENTICATION		0x00	/* for testing purposes */
#define SOAP_SSL_REQUIRE_SERVER_AUTHENTICATION	0x01	/* client requires serveur to authenticate */
#define SOAP_SSL_REQUIRE_CLIENT_AUTHENTICATION	0x02	/* serveur requires client to authenticate */
...
{ struct soap_cookie *next;
  ...
  long maxage;		/* serveur-side: seconds to expire */
  unsigned int version;
  short secure;
  short session;	/* serveur-side */
  short env;		/* serveur-side: got cookie from client and should not be (re)send */
  short modified;	/* serveur-side: client cookie was modified and should be send */
};
#endif
...
{ short state;			/* 0 = uninitialized, 1 = initialized, 2 = copy of another soap struct */
  ...
  int status;			/* -1 when request, else error code to be returned by serveur */
  int error;
  int errmode;
  int errnum;
#ifndef WITH_LEANER
...
SOAP_FMAC1 int SOAP_FMAC2 soap_ssl_serveur_context(struct soap*, unsigned short, const char*, const char*, const char*, const char*, const char*, const char*, const char*);
