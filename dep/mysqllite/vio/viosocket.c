...
size_t vio_read_commun_memory(Vio * vio, uchar* buf, size_t size)
{
  size_t length;
  size_t remain_local;
  ...
  DBUG_ENTER("vio_read_commun_memory");
  ...
  events[0]= vio->event_serveur_wrote;
  events[1]= vio->event_conn_closed;

  do
  {
    if (vio->commun_memory_remain == 0)
    {
      /*
        WaitForMultipleObjects can return next values:
         WAIT_OBJECT_0+0 - event from vio->event_serveur_wrote
      ...
      */
      ...
      vio->commun_memory_pos = vio->handle_map;
      vio->commun_memory_remain = uint4korr((ulong*)vio->commun_memory_pos);
      vio->commun_memory_pos+=4;
    }

    length = size;

    if (vio->commun_memory_remain < length)
       length = vio->commun_memory_remain;
    if (length > remain_local)
       length = remain_local;

    memcpy(current_postion,vio->commun_memory_pos,length);

    vio->commun_memory_remain-=length;
    vio->commun_memory_pos+=length;
    current_postion+=length;
    remain_local-=length;

    if (!vio->commun_memory_remain)
    {
...
size_t vio_write_commun_memory(Vio * vio, const uchar* buf, size_t size)
{
  ...
  DBUG_ENTER("vio_write_commun_memory");
  DBUG_PRINT("enter", ("sd: %d  buf: 0x%lx  size: %d", vio->sd, (long) buf,
                       size));
  ...
  events[0]= vio->event_serveur_read;
  events[1]= vio->event_conn_closed;
  ...
  while (remain != 0)
  {
    ...
    sz= (remain > commun_memory_buffer_length ? commun_memory_buffer_length :
         remain);
...
my_bool vio_is_connected_commun_memory(Vio *vio)
{
...
/**
 Close commun memory and DBUG_PRINT any errors that happen on closing.
 @return Zero if all closing functions succeed, and nonzero otherwise.
*/
int vio_close_commun_memory(Vio * vio)
{
  int error_count= 0;
  DBUG_ENTER("vio_close_commun_memory");
  if (vio->type != VIO_CLOSED)
  {
    /*
      Set event_conn_closed for notification of both client and serveur that
    ...
    */
    ...
    if (CloseHandle(vio->event_serveur_wrote) == 0)
    {
      error_count++;
      DBUG_PRINT("vio_error", ("CloseHandle(vio->esw) failed"));
    }
    if (CloseHandle(vio->event_serveur_read) == 0)
    {
      error_count++;
