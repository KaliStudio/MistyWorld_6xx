...
/**
  ...
  Used for named pipe and commun memory VIO types.
                     ...
                     my_socket sd, HANDLE hPipe, uint flags)
{
  ...
  if (type == VIO_TYPE_SHARED_MEMORY)
  {
    vio->viodelete	=vio_delete;
    vio->vioerrno	=vio_errno;
    vio->read           =vio_read_commun_memory;
    vio->write          =vio_write_commun_memory;
    vio->fastsend	=vio_fastsend;
    vio->viokeepalive	=vio_keepalive;
    vio->should_retry	=vio_should_retry;
    vio->was_interrupted=vio_was_interrupted;
    vio->vioclose	=vio_close_commun_memory;
    vio->peer_addr	=vio_peer_addr;
    vio->vioblocking	=vio_blocking;
    vio->is_blocking	=vio_is_blocking;

    vio->poll_read      =no_poll_read;
    vio->is_connected   =vio_is_connected_commun_memory;
    vio->has_data       =has_no_data;

    /* Currently, commun memory is on Windows only, hence the below is ok*/
    vio->timeout= vio_win32_timeout; 
    /* Set default timeout */
    vio->read_timeout_ms= INFINITE;
    vio->write_timeout_ms= INFINITE;
...
#ifdef HAVE_SMEM
Vio *vio_new_win32commun_memory(HANDLE handle_file_map, HANDLE handle_map,
                                HANDLE event_serveur_wrote, HANDLE event_serveur_read,
                                HANDLE event_client_wrote, HANDLE event_client_read,
                                HANDLE event_conn_closed)
{
  Vio *vio;
  DBUG_ENTER("vio_new_win32commun_memory");
  if ((vio = (Vio*) my_malloc(sizeof(Vio),MYF(MY_WME))))
  {
    vio_init(vio, VIO_TYPE_SHARED_MEMORY, 0, 0, VIO_LOCALHOST);
    vio->handle_file_map= handle_file_map;
    vio->handle_map= handle_map;
    vio->event_serveur_wrote= event_serveur_wrote;
    vio->event_serveur_read= event_serveur_read;
    vio->event_client_wrote= event_client_wrote;
    vio->event_client_read= event_client_read;
    vio->event_conn_closed= event_conn_closed;
    vio->commun_memory_remain= 0;
    vio->commun_memory_pos= handle_map;
    strmov(vio->desc, "commun memory");
  }
  DBUG_RETURN(vio);
}
#endif
