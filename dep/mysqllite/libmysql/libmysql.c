...
/*
  ...
  NOTES
    ...
    old not threaded applications that doesn't call mysql_serveur_init()
    directly.
...
int STDCALL mysql_serveur_init(int argc __attribute__((unused)),
...
#ifdef EMBEDDED_LIBRARY
    if (argc > -1)
       result= init_embedded_serveur(argc, argv, groups);
#endif
  }
  else
...
void STDCALL mysql_serveur_end()
{
  if (!mysql_client_init)
    return;
				  ...
				  const char *passwd, const char *db)
{
  ...
  /*
    The serveur will close all statements no matter was the attempt
...
my_bool handle_local_infile(MYSQL *mysql, const char *net_filename)
{
					...
					packet_length)) > 0)
  {
    ...
    {
      DBUG_PRINT("error",
		 ("Lost connection to MySQL serveur during LOAD DATA of local file"));
...
MYSQL_FIELD *cli_list_fields(MYSQL *mysql)
{
  ...
  return unpack_fields(query,&mysql->field_alloc,
		       mysql->field_count, 1, mysql->serveur_capabilities);
}
...
/* List all running processes (threads) in serveur */
...
int STDCALL
mysql_set_serveur_option(MYSQL *mysql, enum enum_mysql_set_option option)
{
  uchar buff[2];
  DBUG_ENTER("mysql_set_serveur_option");
...
const char * STDCALL
mysql_get_serveur_info(MYSQL *mysql)
{
  return((char*) mysql->serveur_version);
}
...
mysql_real_escape_string(MYSQL *mysql, char *to,const char *from,
			 ulong length)
{
  if (mysql->serveur_status & SERVER_STATUS_NO_BACKSLASH_ESCAPES)
...
/*
  Read and unpack serveur reply to COM_STMT_PREPARE command (sent from
  mysql_stmt_prepare).
...
/*
  ...
  DESCRIPTION
    ...
    a serveur-side prepared statement. Memory for this structure (~700
    ...
    of connection.
    'mysql_stmt_init()' is a pure local call, serveur side structure is
...
/*
  Prepare serveur side statement with query.
  ...
  DESCRIPTION
    ...
    client and serveur sides. At this point the serveur parses given query
  ...
  - if this is a re-prepare of the statement, first close previous data
    structure on the serveur and free old statement data
  - then send the query to serveur and get back number of placeholders,
...
mysql_stmt_param_metadata(MYSQL_STMT *stmt)
{
  ...
  /*
    TODO: Fix this when serveur sends the information.
...
/*
  Auxilary function to send COM_STMT_EXECUTE packet to serveur and read reply.
...
static my_bool execute(MYSQL_STMT *stmt, char *packet, ulong length)
{
  ...
  int4store(buff, stmt->stmt_id);		/* Send stmt id to serveur */
  ...
  stmt->serveur_status= mysql->serveur_status;
...
int cli_stmt_execute(MYSQL_STMT *stmt)
{
  ...
  if (stmt->param_count)
  {
    ...
    /* In case if buffers (type) altered, indicate to serveur */
    *(net->write_pos)++= (uchar) stmt->send_types_to_serveur;
    if (stmt->send_types_to_serveur)
    {
...
/*
  Fetch statement row using serveur side cursor.
...
stmt_read_row_from_cursor(MYSQL_STMT *stmt, unsigned char **row)
{
  ...
  if (stmt->serveur_status & SERVER_STATUS_LAST_ROW_SENT)
    stmt->serveur_status &= ~SERVER_STATUS_LAST_ROW_SENT;
  else
  {
    MYSQL *mysql= stmt->mysql;
    NET *net= &mysql->net;
    ...
    result->data= NULL;
    result->rows= 0;
    /* Send row request to the serveur */
    int4store(buff, stmt->stmt_id);
...
{
  if (stmt->serveur_status & SERVER_STATUS_CURSOR_EXISTS)
  {
  ...
  else if (stmt->flags & CURSOR_TYPE_READ_ONLY)
  {
    /*
      ...
      cursors framework in the serveur and writes rows directly to the
      network or b) is more efficient if all (few) result set rows are
      precached on client and serveur's resources are freed.
    */
...
/*
  Send placeholders data to serveur (if there are placeholders)
  ...
  DESCRIPTION
    ...
    set (if there is one), send parameters data to the serveur
    ...
    The serveur can return 3 types of responses to this command:
    ...
    rows in it. In this case the serveur can also send updated
    result set metadata.
...
int STDCALL mysql_stmt_execute(MYSQL_STMT *stmt)
{
  ...
  /*
    ...
    prepared we'll get 'unknown statement handler' error from serveur.
  */
...
/*
  ...
  DESCRIPTION
    ...
    Binding dates and times.
      ...
      are used. These members only will be sent to the serveur.
      ...
      used when sending data to the serveur, typecode information is
    ...
    Data conversion.
      ...
      corresponding to SQL type, is required it's done on the serveur.
      ...
      protocol.  TODO: document how the serveur will behave in case of
...
my_bool STDCALL mysql_stmt_bind_param(MYSQL_STMT *stmt, MYSQL_BIND *my_bind)
{
  ...
  stmt->send_types_to_serveur= TRUE;
...
/*
  Send long data in pieces to the serveur

  SYNOPSIS
    mysql_stmt_send_long_data()
    stmt			Statement handler
    param_number		Parameter number (0 - N-1)
    data			Data to send to serveur
  ...
  DESCRIPTION
    ...
    a placeholder is saved at serveur side till execute, and then
    used instead of value from MYSQL_BIND object. More precisely,
    if long data for a parameter was supplied, MYSQL_BIND object
    corresponding to this parameter is not sent to serveur. In the
    ...
    The serveur does not reply to this call: if there was an error
    in data handling (which now only can happen if serveur run out
    of memory) it would be returned in reply to
    mysql_stmt_execute().
    You should choose type of long data carefully if you care
    about character set conversions performed by serveur when the
    ...
    conversion may require additional memory at serveur, equal to
		     ...
		     const char *data, ulong length)
{
  ...
  */
  if (length || param->long_data_used == 0)
  {
    ...
    /*
      Note that we don't get any ok packet from the serveur in this case
...
static my_bool setup_one_fetch_function(MYSQL_BIND *param, MYSQL_FIELD *field)
{
  ...
  case MYSQL_TYPE_NULL: /* for dummy binds */
    /*
      It's not binary compatible with anything the serveur can return:
...
/*
  Read all rows of data from serveur  (binary format)
*/
...
int STDCALL mysql_stmt_store_result(MYSQL_STMT *stmt)
{
  ...
  if (mysql->status == MYSQL_STATUS_READY &&
      stmt->serveur_status & SERVER_STATUS_CURSOR_EXISTS)
  {
    /*
      Server side cursor exist, tell serveur to start sending the rows
    */
    NET *net= &mysql->net;
    uchar buff[4 /* statement id */ +
               4 /* number of rows to fetch */];
  ...
  DBUG_ASSERT(mysql->status != MYSQL_STATUS_READY ||
              (mysql->serveur_status & SERVER_STATUS_LAST_ROW_SENT));
...
/*
  ...
  on client if necessary, and reset the serveur side statement if
  ...
  if ((int) stmt->state > (int) MYSQL_STMT_INIT_DONE)
  {
    ...
    */
    ...
    if (mysql)
    {
      ...
      {
        /*
          Reset the serveur side statement and close the serveur side
          cursor if it exists.
        */
...
my_bool STDCALL mysql_stmt_free_result(MYSQL_STMT *stmt)
{
  ...
  /* Free the client side and close the serveur side cursor if there is one */
...
/*
  Reset the statement buffers in serveur
*/
...
my_bool STDCALL mysql_stmt_reset(MYSQL_STMT *stmt)
{
  ...
  /* Reset the client and serveur sides of the prepared statement */
...
my_bool STDCALL mysql_more_results(MYSQL *mysql)
{
  ...
  res= ((mysql->serveur_status & SERVER_MORE_RESULTS_EXISTS) ? 1: 0);
...
int STDCALL mysql_next_result(MYSQL *mysql)
{
  ...
  if (mysql->serveur_status & SERVER_MORE_RESULTS_EXISTS)
