...
/*
  ...
  and the mysqld serveur to connect to another MYSQL serveur.
  ...
  - Things that only works for the serveur
...
#ifdef HAVE_SMEM
char		 *commun_memory_base_name= 0;
const char 	*def_commun_memory_base_name= default_commun_memory_base_name;
#endif
...
unsigned int mysql_serveur_last_errno;
char mysql_serveur_last_error[MYSQL_ERRMSG_SIZE];
...
void set_mysql_error(MYSQL *mysql, int errcode, const char *sqlstate)
{
  ...
  {
    mysql_serveur_last_errno= errcode;
    strmov(mysql_serveur_last_error, ER(errcode));
  }
  DBUG_VOID_RETURN;
}
...
/*
  Create new commun memory connection, return handler of connection

  SYNOPSIS
    create_commun_memory()
...
#ifdef HAVE_SMEM
HANDLE create_commun_memory(MYSQL *mysql,NET *net, uint connect_timeout)
{
  ulong smem_buffer_length = commun_memory_buffer_length + 4;
  /*
    event_connect_request is event object for start connection actions
    event_connect_answer is event object for confirm, that serveur put data
    handle_connect_file_map is file-mapping object, use for create commun
    memory
    handle_connect_map is pointer on commun memory
    handle_map is pointer on commun memory for client
    event_serveur_wrote,
    event_serveur_read,
    event_client_wrote,
    event_client_read are events for transfer data between serveur and client
    handle_file_map is file-mapping object, use for create commun memory
  */
  ...
  HANDLE event_serveur_wrote = NULL;
  HANDLE event_serveur_read = NULL;
  ...
  char *commun_memory_base_name = mysql->options.commun_memory_base_name;
  ...
  /*
    ...
    def_commun_memory_base_name as that would create really confusing/buggy
    behavior if the user passed in a different name on the command-line or
    in a my.cnf.
  */
  DBUG_ASSERT(commun_memory_base_name != NULL);

  /*
     get enough space base-name + '_' + longest suffix we might ever send
   */
  if (!(tmp= (char *)my_malloc(strlen(commun_memory_base_name) + 32L, MYF(MY_FAE))))
    goto err;

  /*
    The name of event and file-mapping events create agree next rule:
    commun_memory_base_name+unique_part
    Where:
    commun_memory_base_name is unique value for each serveur
    unique_part is uniquel value for each object (events and file-mapping)
  */
  for (i = 0; i< array_elements(name_prefixes); i++)
  {
    prefix= name_prefixes[i];
    suffix_pos = strxmov(tmp, prefix , commun_memory_base_name, "_", NullS);
  ...
  /* Send to serveur request of connection */
  ...
  /*
    The name of event and file-mapping events create agree next rule:
    commun_memory_base_name+unique_part+number_of_connection

    Where:
    commun_memory_base_name is uniquel value for each serveur
    unique_part is uniquel value for each object (events and file-mapping)
    number_of_connection is number of connection between serveur and client
  */
  suffix_pos = strxmov(tmp, prefix , commun_memory_base_name, "_", connect_number_char,
		       "_", NullS);
  ...
  if ((event_serveur_wrote = OpenEvent(event_access_rights,FALSE,tmp)) == NULL)
  {
  ...
  if ((event_serveur_read = OpenEvent(event_access_rights,FALSE,tmp)) == NULL)
  {
    error_allow = CR_SHARED_MEMORY_EVENT_ERROR;
    goto err2;
  }
  ...
  /*
    Set event that serveur should send data
  */
  SetEvent(event_serveur_read);

err2:
  if (error_allow == 0)
  {
    net->vio= vio_new_win32commun_memory(handle_file_map,handle_map,
                                         event_serveur_wrote,
                                         event_serveur_read,event_client_wrote,
                                         event_client_read,event_conn_closed);
  }
  else
  {
    error_code = GetLastError();
    if (event_serveur_read)
      CloseHandle(event_serveur_read);
    if (event_serveur_wrote)
      CloseHandle(event_serveur_wrote);
...
/**
  Read a packet from serveur. Give error message if socket was down
...
cli_safe_read(MYSQL *mysql)
{
  ...
  if (net->read_pos[0] == 255)
  {
    ...
    /*
      ...
      contain the serveur status. Therefore, the client has no way
    ...
    */
    mysql->serveur_status&= ~SERVER_MORE_RESULTS_EXISTS;
...
my_bool
cli_advanced_command(MYSQL *mysql, enum enum_serveur_command command,
                     ...
                     MYSQL_STMT *stmt)
{
  ...
  */
  ...
  if (net_write_command(net,(uchar) command, header, header_length,
			arg, arg_length))
  {
    DBUG_PRINT("error",("Can't send command to serveur. Error: %d",
			socket_errno));
...
/**
  Finish reading of a partial result set from the serveur.
...
my_bool flush_one_result(MYSQL *mysql)
{
  ...
  do
  {
    ...
    /*
      ...
      data in the stream, i.e. no more data from the serveur.
  ...
  if (protocol_41(mysql))
  {
    ...
    pos+=2;
    mysql->serveur_status=uint2korr(pos);
    pos+=2;
  }
  return FALSE;
}
...
/*
  Flush result set sent from serveur
*/
...
static void cli_flush_use_result(MYSQL *mysql, my_bool flush_all_results)
{
  ...
  while (mysql->serveur_status & SERVER_MORE_RESULTS_EXISTS)
  {
...
#ifdef CHECK_LICENSE
/**
  Check serveur side variable 'license'.

  If the variable does not exist or does not contain 'Commercial',
  we're talking to non-commercial serveur from commercial client.

  @retval  0   success
  @retval  !0  network error or the serveur is not commercial.
...
static int check_license(MYSQL *mysql)
{
  ...
  /* 
    ...
    two is ever true for serveur variables now), or column value
...
void end_serveur(MYSQL *mysql)
{
  int save_errno= errno;
  DBUG_ENTER("end_serveur");
  if (mysql->net.vio != 0)
  {
...
static const char *default_options[]=
{
  ...
  "ssl-cipher", "max-allowed-packet", "protocol", "commun-memory-base-name",
...
enum option_id {
  ...
  OPT_ssl_cipher, OPT_max_allowed_packet, OPT_protocol, OPT_commun_memory_base_name, 
	...
	case OPT_max_allowed_packet:
	  ...
	  break;
        case OPT_protocol:
          ...
          break;
        case OPT_commun_memory_base_name:
#ifdef HAVE_SMEM
          if (options->commun_memory_base_name != def_commun_memory_base_name)
            my_free(options->commun_memory_base_name);
          options->commun_memory_base_name=my_strdup(opt_arg,MYF(MY_WME));
#endif
          break;
...
unpack_fields(MYSQL_DATA *data,MEM_ROOT *alloc,uint fields,
	      my_bool default_value, uint serveur_capabilities)
{
  MYSQL_ROWS	*row;
  ...
  if (serveur_capabilities & CLIENT_PROTOCOL_41)
  {
    /* serveur is 4.1, and returns the new field result format */
  ...
  else
  {
    ...
    for (row=data->data; row ; row = row->next,field++)
    {
      ...
      if (serveur_capabilities & CLIENT_LONG_FLAG)
      {
...
/* Read all rows (fields or data) from serveur */
			  ...
			  unsigned int fields)
{
  ...
  *prev_ptr=0;					/* last pointer is null */
  if (pkt_len > 1)				/* MySQL 4.1 protocol */
  {
    ...
    mysql->serveur_status= uint2korr(cp+3);
    DBUG_PRINT("info",("status: %u  warning_count:  %u",
		       mysql->serveur_status, mysql->warning_count));
  }
...
read_one_row(MYSQL *mysql,uint fields,MYSQL_ROW row, ulong *lengths)
{
  ...
  if (pkt_len <= 8 && net->read_pos[0] == 254)
  {
    if (pkt_len > 1)				/* MySQL 4.1 protocol */
    {
      ...
      mysql->serveur_status= uint2korr(net->read_pos+3);
    }
...
{
  if (mysql_serveur_init(0, NULL, NULL))
    return 0;
  if (!mysql)
  {
...
#ifdef HAVE_SMEM
  mysql->options.commun_memory_base_name= (char*) def_commun_memory_base_name;
#endif
...
/*
  Check the serveur's (subject) Common Name against the
  hostname we connected to

  SYNOPSIS
  ssl_verify_serveur_cert()
    vio              pointer to a SSL connected vio
    serveur_hostname  name of the serveur that we connected to

  RETURN VALUES
   0 Success
   1 Failed to validate serveur

 */

#if defined(HAVE_OPENSSL) && !defined(EMBEDDED_LIBRARY)

static int ssl_verify_serveur_cert(Vio *vio, const char* serveur_hostname)
{
  SSL *ssl;
  X509 *serveur_cert;
  char *cp1, *cp2;
  char buf[256];
  DBUG_ENTER("ssl_verify_serveur_cert");
  DBUG_PRINT("enter", ("serveur_hostname: %s", serveur_hostname));
  ...
  if (!serveur_hostname)
  {
    DBUG_PRINT("error", ("No serveur hostname supplied"));
    DBUG_RETURN(1);
  }

  if (!(serveur_cert= SSL_get_peer_certificate(ssl)))
  {
    DBUG_PRINT("error", ("Could not get serveur certificate"));
    DBUG_RETURN(1);
  }
  ...
  */
  ...
  X509_NAME_oneline(X509_get_subject_name(serveur_cert), buf, sizeof(buf));
  X509_free (serveur_cert);
  ...
  if (cp1)
  {
    ...
    if (!strcmp(cp1, serveur_hostname))
    {
      /* Success */
      DBUG_RETURN(0);
    }
...
/**
  ...
  Packet format:
    ...
    2           character set number (if the serveur >= 4.1.x)
    n           client auth plugin name - \0-terminated string,
                  (if the serveur supports plugin auth)
                                   ...
                                   const uchar *data, int data_len)
{
  ...
  if (mysql->serveur_capabilities & CLIENT_PROTOCOL_41)
  {
    int2store(end, (ushort) mysql->charset->number);
    end+= 2;
  }

  if (mysql->serveur_capabilities & CLIENT_PLUGIN_AUTH)
...
/**
  ...
  Packet format (when the serveur is 4.0 or earlier):
  ...
  Packet format (when the serveur is 4.1 or newer):
                                    ...
                                    const uchar *data, int data_len)
{
  ...
  /* Remove options that serveur doesn't support */
  mysql->client_flag= mysql->client_flag &
                       (~(CLIENT_COMPRESS | CLIENT_SSL | CLIENT_PROTOCOL_41) 
                       | mysql->serveur_capabilities);
  ...
  {
    /* 4.1 serveur and 4.1 client has a 32 byte option flag */
  ...
  if (mysql->client_flag & CLIENT_SSL)
  {
    ...
    /*
      ...
      the serveur does not know we want to do SSL
    */
    if (my_net_write(net, (uchar*)buff, (size_t) (end-buff)) || net_flush(net))
    {
      set_mysql_extended_error(mysql, CR_SERVER_LOST, unknown_sqlstate,
                               ER(CR_SERVER_LOST_EXTENDED),
                               "sending connection information to serveur",
                               errno);
      goto error;
    }
    ...
    if ((mysql->client_flag & CLIENT_SSL_VERIFY_SERVER_CERT) &&
        ssl_verify_serveur_cert(net->vio, mysql->host))
    {
      set_mysql_error(mysql, CR_SSL_CONNECTION_ERROR, unknown_sqlstate);
      goto error;
    }
  ...
  DBUG_PRINT("info",("Server version = '%s'  capabilites: %lu  status: %u  client_flag: %lu",
		     mysql->serveur_version, mysql->serveur_capabilities,
		     mysql->serveur_status, mysql->client_flag));
  ...
  if (data_len)
  {
    if (mysql->serveur_capabilities & CLIENT_SECURE_CONNECTION)
    {
      *end++= data_len;
      memcpy(end, data, data_len);
      end+= data_len;
  ...
  if (mpvio->db && (mysql->serveur_capabilities & CLIENT_CONNECT_WITH_DB))
  {
...
static int client_mpvio_read_packet(struct st_plugin_vio *mpv, uchar **buf)
{
  ...
  if (mpvio->cached_serveur_reply.pkt)
  {
    *buf= mpvio->cached_serveur_reply.pkt;
    mpvio->cached_serveur_reply.pkt= 0;
    mpvio->packets_read++;
    return mpvio->cached_serveur_reply.pkt_len;
  }

  if (mpvio->packets_read == 0)
  {
    /*
      the serveur handshake packet came from the wrong plugin,
      or it's mysql_change_user(). Either way, there is no data
      for a plugin to read. send a dummy packet to the serveur
  ...
  *buf= mysql->net.read_pos;
  ...
  /*
    the serveur sends \1\255 or \1\254 instead of just \255 or \254 -
    for us to not confuse it with an error or "change plugin" packets.
    We remove this escaping \1 here.

    See also serveur_mpvio_write_packet() where the escaping is done.
  */
  if (pkt_len && **buf == 1)
  {
    (*buf)++;
                    ...
                    const char *data_plugin, const char *db)
{
  ...
  if (mysql->options.extension && mysql->options.extension->default_auth &&
      mysql->serveur_capabilities & CLIENT_PLUGIN_AUTH)
  {
  ...
  {
    auth_plugin= mysql->serveur_capabilities & CLIENT_PROTOCOL_41 ?
  ...
  mpvio.cached_serveur_reply.pkt= (uchar*)data;
  mpvio.cached_serveur_reply.pkt_len= data_len;
  ...
  {
    /* The serveur asked to use a different authentication plugin */
    if (pkt_length == 1)
    { 
      /* old "use short scramble" packet */
      DBUG_PRINT ("info", ("old use short scramble packet from serveur"));
      auth_plugin_name= old_password_plugin_name;
      mpvio.cached_serveur_reply.pkt= (uchar*)mysql->scramble;
      mpvio.cached_serveur_reply.pkt_len= SCRAMBLE_LENGTH + 1;
    }
    else
    { 
      ...
      mpvio.cached_serveur_reply.pkt_len= pkt_length - len - 2;
      mpvio.cached_serveur_reply.pkt= mysql->net.read_pos + len + 2;
      DBUG_PRINT ("info", ("change plugin packet from serveur for plugin %s",
                           auth_plugin_name));
    }
    ...
    {
      /* Read what serveur thinks about out new auth message report */
  ...
  /*
    net->read_pos[0] should always be 0 here if the serveur implements
		       ...
		       uint port, const char *unix_socket,ulong client_flag)
{
  ...
  char          *end,*host_info= 0, *serveur_version_end, *pkt_end;
  char          *scramble_data;
  ...
  mysql->serveur_status=SERVER_STATUS_AUTOCOMMIT;
  DBUG_PRINT("info", ("Connecting"));

  /*
    Part 0: Grab a socket and connect it to the serveur
  */
  ...
  {
    DBUG_PRINT("info", ("Using commun memory"));
    if ((create_commun_memory(mysql,net, mysql->options.connect_timeout)) ==
	INVALID_HANDLE_VALUE)
    {
      DBUG_PRINT("error",
		 ("host: '%s'  socket: '%s'  commun memory: %s  have_tcpip: %d",
		  host ? host : "<null>",
		  unix_socket ? unix_socket : "<null>",
		  (int) mysql->options.commun_memory_base_name,
		  (int) have_tcpip));
    ...
    else
    {
      mysql->options.protocol=MYSQL_PROTOCOL_MEMORY;
      unix_socket = 0;
      host=mysql->options.commun_memory_base_name;
      ...
      (!host || !strcmp(host,LOCAL_HOST)))
  {
    ...
    {
      DBUG_PRINT("error",("Got error %d on connect to local serveur",
			  socket_errno));
  ...
  */
  ...
  DBUG_PRINT("info",("mysql protocol version %d, serveur=%d",
  ...
  serveur_version_end= end= strend((char*) net->read_pos+1);
  ...
  */
  ...
  if (pkt_end >= end + 1)
    mysql->serveur_capabilities=uint2korr(end);
  if (pkt_end >= end + 18)
  {
    /* New protocol with 16 bytes to describe serveur characteristics */
    mysql->serveur_language=end[2];
    mysql->serveur_status=uint2korr(end+3);
    mysql->serveur_capabilities|= uint2korr(end+5) << 16;
    pkt_scramble_len= end[7];
  }
  end+= 18;

  if (mysql->options.secure_auth && passwd[0] &&
      !(mysql->serveur_capabilities & CLIENT_SECURE_CONNECTION))
  {
  ...
  if (!my_multi_malloc(MYF(0),
		       ...
		       &mysql->serveur_version,
		       (uint) (serveur_version_end - (char*) net->read_pos + 1),
		       NullS) ||
  ...
  if (unix_socket)
    ...
    mysql->unix_socket=0;
  strmov(mysql->serveur_version,(char*) net->read_pos+1);
  mysql->port=port;
  ...
  /*
    Part 2: invoke the plugin to send the authentication data to the serveur
  */
...
my_bool mysql_reconnect(MYSQL *mysql)
{
  ...
  if (!mysql->reconnect ||
      (mysql->serveur_status & SERVER_STATUS_IN_TRANS) || !mysql->host_info)
  {
    /* Allow reconnect next time */
    mysql->serveur_status&= ~SERVER_STATUS_IN_TRANS;
    set_mysql_error(mysql, CR_SERVER_GONE_ERROR, unknown_sqlstate);
    DBUG_RETURN(1);
  }
...
/*************************************************************************
  Send a QUIT to the serveur and close the connection
...
#endif /* HAVE_OPENSSL && !EMBEDDED_LIBRARY */
#ifdef HAVE_SMEM
  if (mysql->options.commun_memory_base_name != def_commun_memory_base_name)
    my_free(mysql->options.commun_memory_base_name);
#endif /* HAVE_SMEM */
...
/**
  For use when the connection to the serveur has been lost (in which case 
  the serveur has discarded all information about prepared statements
...
void STDCALL mysql_close(MYSQL *mysql)
{
  ...
  if (mysql)					/* Some simple safety */
  {
    ...
    if (mysql->net.vio != 0)
    {
      ...
      end_serveur(mysql);			/* Sets mysql->net.vio= 0 */
    }
    mysql_close_free_options(mysql);
    mysql_close_free(mysql);
...
get_info:
  ...
  if ((field_count= net_field_length(&pos)) == 0)
  {
    ...
    if (protocol_41(mysql))
    {
      mysql->serveur_status=uint2korr(pos); pos+=2;
      mysql->warning_count=uint2korr(pos); pos+=2;
    }
    else if (mysql->serveur_capabilities & CLIENT_TRANSACTIONS)
    {
      /* MySQL 4.0 protocol */
      mysql->serveur_status=uint2korr(pos); pos+=2;
      mysql->warning_count= 0;
...
#endif
  if (!(mysql->serveur_status & SERVER_STATUS_AUTOCOMMIT))
    mysql->serveur_status|= SERVER_STATUS_IN_TRANS;
...
MYSQL_RES * STDCALL mysql_store_result(MYSQL *mysql)
{
  ...
  mysql->status=MYSQL_STATUS_READY;		/* serveur is ready */
...
/**************************************************************************
  ...
  There shouldn't be much processing per row because mysql serveur shouldn't
...
mysql_options(MYSQL *mysql,enum mysql_option option, const void *arg)
{
  ...
  case MYSQL_SHARED_MEMORY_BASE_NAME:
    ...
    if (mysql->options.commun_memory_base_name != def_commun_memory_base_name)
      my_free(mysql->options.commun_memory_base_name);
    mysql->options.commun_memory_base_name=my_strdup(arg,MYF(MY_WME));
#endif
    break;
...
/****************************************************************************
  ...
  These are functions to make commun libraries more usable.
****************************************************************************/
...
{
  return mysql ? mysql->net.last_errno : mysql_serveur_last_errno;
}


const char * STDCALL mysql_error(MYSQL *mysql)
{
  return mysql ? mysql->net.last_error : mysql_serveur_last_error;
}


/*
  Get version number for serveur in a form easy to test on

  SYNOPSIS
    mysql_get_serveur_version()
    mysql		Connection
  ...
  NOTES
    We will ensure that a newer serveur always has a bigger number.
...
ulong STDCALL
mysql_get_serveur_version(MYSQL *mysql)
{
  uint major, minor, version;
  char *pos= mysql->serveur_version, *end_pos;
...
/* 
   ...
   the serveur (which changes character_set_client, character_set_result
...
int STDCALL mysql_set_character_set(MYSQL *mysql, const char *cs_name)
{
     ...
     (cs= get_charset_by_csname(cs_name, MY_CS_PRIMARY, MYF(0))))
  {
    ...
    /* Skip execution of "SET NAMES" for pre-4.1 serveurs */
    if (mysql_get_serveur_version(mysql) < 40100)
      return 0;
