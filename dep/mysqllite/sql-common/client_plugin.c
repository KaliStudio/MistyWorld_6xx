...
/**
  ...
  Client plugins are somewhat different from serveur plugins, they are simpler.

  They do not need to be installed or in any way explicitly loaded on the
  client, they are loaded automatically on demand.
  One client plugin per commun object, soname *must* match the plugin name.
...
/**
  ...
  @param dlhandle       a handle to the commun object (returned by dlopen)
