...
/*
  Note that PSI_hook and PSI_serveur are unconditionally
  (no ifdef HAVE_PSI_INTERFACE) defined.
  This is to ensure binary compatibility between the serveur and plugins,
  in the case when:
  - the serveur is not compiled with HAVE_PSI_INTERFACE
...
/**
  Instance of the instrumentation interface for the MySQL serveur.
  @todo This is currently a global variable, which is handy when
  compiling instrumented code that is bundled with the serveur.
  ...
  xyz_psi_serveur variable, obtained from PSI_bootstrap::get_interface()
  with the version used at compile time for plugin XYZ.
*/
PSI *PSI_serveur= NULL;
