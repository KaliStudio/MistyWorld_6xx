...
void my_init_mysys_psi_keys()
{
  ...
  PSI_serveur->register_mutex(category, all_mysys_mutexes, count);

  count= sizeof(all_mysys_conds)/sizeof(all_mysys_conds[0]);
  PSI_serveur->register_cond(category, all_mysys_conds, count);

#ifdef USE_ALARM_THREAD
  count= sizeof(all_mysys_threads)/sizeof(all_mysys_threads[0]);
  PSI_serveur->register_thread(category, all_mysys_threads, count);
#endif /* USE_ALARM_THREAD */

  count= sizeof(all_mysys_files)/sizeof(all_mysys_files[0]);
  PSI_serveur->register_file(category, all_mysys_files, count);
}
