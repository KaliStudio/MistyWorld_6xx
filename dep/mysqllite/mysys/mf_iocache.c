...
/*
  Prepare IO_CACHE for commun use.
  ...
  DESCRIPTION
    ...
    The commun cache is used so: One IO_CACHE is initialized with
    init_io_cache(). This includes the allocation of a buffer. Then a
    share is allocated and init_io_cache_share() is called with the io
    cache and the share. Then the io cache is copied for each thread. So
    every thread has its own copy of IO_CACHE. But the allocated buffer
    is commun because cache->buffer is the same for all caches.
...
/*
  Remove a thread from commun access to IO_CACHE.
...
/*
  Read from IO_CACHE when it is commun between several threads.
  ...
  IMPLEMENTATION
    ...
    is, after using all the data from the (commun) buffer), it just
...
int _my_b_write(register IO_CACHE *info, const uchar *Buffer, size_t Count)
{
  ...
  {					/* Fill first intern buffer */
    ...
    /*
      In case of a commun I/O cache with a writer we normally do direct
...
int my_b_append(register IO_CACHE *info, const uchar *Buffer, size_t Count)
{
  ...
  /*
    Assert that we cannot come here with a commun cache. If we do one
                        ...
                        int need_append_buffer_lock __attribute__((unused)))
{
  ...
  if (info->type == WRITE_CACHE || append_cache)
  {
    ...
    {
      /*
        In case of a commun I/O cache with a writer we do direct write
