                ...
                int timeout_sec)
{
  ...
  if (locktype == F_UNLCK)
  {
    ...
    */
    ...
    /* read lock is mapped to a commun lock. */
    dwFlags= 0;
  else
  ...
  /*
    ...
    exclusively, then additionally commun, then did one unlock, and
    ...
    wants to convert an exclusive lock into a commun one, it will now
    first unlock the file and then lock it commun. A waiting exclusive
