...
/*
  ...
  1. Threads that want to do a resize wait there if in_resize is set.
     This is not used in the serveur. The serveur refuses a second resize
                     ...
                     uint age_threshold)
{
  ...
  /*
    ...
    already. This cannot happen in the MySQL serveur though. It allows
...
/*
  ...
  NOTES.
    ...
    dont_write is always TRUE in the serveur (info->lock_type is never F_UNLCK).
*/
    ...
    */
	...
	goto no_key_cache;
      ...
      */
      ...
      if (!dont_write)
      {
        /* Not used in the serveur. buff has been written to disk at start. */
        if ((block->status & BLOCK_CHANGED) &&
            (!offset && read_length >= keycache->key_cache_block_size))
             link_to_file_list(keycache, block, block->hash_link->file, 1);
      }
