...
/**
  ...
  This file is the net layer API for the MySQL client/serveur protocol.
...
/*
  ...
  client and the serveur.

  This gives an error if a too big packet is found.
  The serveur can change this, but because the client can't normally do this
...
my_bool net_realloc(NET *net, size_t length)
{
  ...
  */
                                  ...
                                  MYF(MY_WME))))
  {
    ...
    /* In the serveur the error is reported by MY_WME flag. */
    DBUG_RETURN(1);
  }
...
/*****************************************************************************
** Write something to serveur/client buffer
*****************************************************************************/
...
/**
  ...
  @param net		NET handler
  @param command	Command in MySQL serveur (enum enum_serveur_command)
...
/*****************************************************************************
** Read something from serveur/clinet
*****************************************************************************/
	...
	if (net->buff[net->where_b + 3] != (uchar) net->pkt_nr)
	{
	  if (net->buff[net->where_b] != (uchar) 255)
	  {
	    DBUG_PRINT("error",
			...
			net->pkt_nr));
            /* 
              We don't make noise serveur side, since the client is expected
              to break the protocol for e.g. --send LOAD DATA .. LOCAL where
              the serveur expects the client to send a file, but the client
...
/**
  Read a packet from the client/serveur and return it without the internal
  package header.
