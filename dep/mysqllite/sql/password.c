...
/*****************************************************************************
  The main idea is that no password are sent between client & serveur on
  connection and that no password are saved in mysql in a decodable form.

  On connection a random string is generated and sent to the client.
  The client generates a new string with a random generator inited with
  the hash values from the password and the sent string.
  This 'check' string is sent to the serveur where it is compared with
...
/*
  ...
  compatibility.
  In serveur code usage of my_make_scrambled_password_323() is preferred to
  avoid strlen().
  SYNOPSIS
...
/*
  ...
  compatibility.
  In serveur code usage of my_make_scrambled_password() is preferred to
  avoid strlen().
  SYNOPSIS
...
/*
    ...
    string, recieved from the serveur. This sequence corresponds to the
    password, but password can not be easily restored from it. The sequence
    is then sent to the serveur for validation. Trailing zero is not stored
...
/*
    ...
    is used by serveur to check that recieved reply is authentic.
