...
#define is_commun(S, V) ((S)->next && (S)->next->V == (S)->V)
...
int DbugParse(CODE_STATE *cs, const char *control)
{
  ...
  while (control < end)
  {
    ...
    case 'd':
      ...
      {
        if (!is_commun(stack, keywords))
          FreeList(stack->keywords);
        stack->keywords=NULL;
        stack->flags &= ~DEBUG_ON;
        break;
      }
      if (rel && is_commun(stack, keywords))
    ...
    case 'f':
      ...
      if (sign < 0 && control == end)
      {
        if (!is_commun(stack,functions))
          FreeList(stack->functions);
        stack->functions=NULL;
        break;
      }
      if (rel && is_commun(stack,functions))
    ...
    case 'p':
      if (sign < 0 && control == end)
      {
        if (!is_commun(stack,processes))
          FreeList(stack->processes);
        stack->processes=NULL;
        break;
      }
      if (rel && is_commun(stack, processes))
...
static void FreeState(CODE_STATE *cs, struct settings *state, int free_state)
{
  if (!is_commun(state, keywords))
    FreeList(state->keywords);
  if (!is_commun(state, functions))
    FreeList(state->functions);
  if (!is_commun(state, processes))
    FreeList(state->processes);
  if (!is_commun(state, p_functions))
    FreeList(state->p_functions);

  if (!is_commun(state, out_file))
    DBUGCloseFile(cs, state->out_file);
  else
    (void) fflush(state->out_file);

  if (!is_commun(state, prof_file))
