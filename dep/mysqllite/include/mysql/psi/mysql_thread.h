...
{
#ifdef HAVE_PSI_INTERFACE
  that->m_psi= PSI_serveur ? PSI_serveur->init_mutex(key, &that->m_mutex)
                          : NULL;
#else
  that->m_psi= NULL;
#endif
...
{
#ifdef HAVE_PSI_INTERFACE
  if (likely(PSI_serveur && that->m_psi))
  {
    PSI_serveur->destroy_mutex(that->m_psi);
    that->m_psi= NULL;
  }
#endif
#ifdef SAFE_MUTEX
  ...
  )
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    locker= PSI_serveur->get_thread_mutex_locker(&state, that->m_psi, PSI_MUTEX_LOCK);
    if (likely(locker != NULL))
      PSI_serveur->start_mutex_wait(locker, src_file, src_line);
  }
#endif
#ifdef SAFE_MUTEX
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_mutex_wait(locker, result);
#endif
  return result;
}
  ...
  )
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    locker= PSI_serveur->get_thread_mutex_locker(&state, that->m_psi, PSI_MUTEX_TRYLOCK);
    if (likely(locker != NULL))
      PSI_serveur->start_mutex_wait(locker, src_file, src_line);
  }
#endif
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(PSI_serveur && that->m_psi))
    PSI_serveur->unlock_mutex(that->m_psi);
#endif
#ifdef SAFE_MUTEX
...
{
#ifdef HAVE_PSI_INTERFACE
  that->m_psi= (PSI_serveur ? PSI_serveur->init_rwlock(key, &that->m_rwlock)
                           : NULL);
#else
  that->m_psi= NULL;
#endif
...
{
#ifdef HAVE_PSI_INTERFACE
  that->m_psi= (PSI_serveur ? PSI_serveur->init_rwlock(key, &that->m_prlock)
                           : NULL);
#else
  that->m_psi= NULL;
#endif
  ...
  mysql_rwlock_t *that)
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    PSI_serveur->destroy_rwlock(that->m_psi);
    that->m_psi= NULL;
  }
#endif
  ...
  )
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    locker= PSI_serveur->get_thread_rwlock_locker(&state, that->m_psi,
                                                 PSI_RWLOCK_READLOCK);
    if (likely(locker != NULL))
      PSI_serveur->start_rwlock_rdwait(locker, src_file, src_line);
  }
#endif
  result= rw_rdlock(&that->m_rwlock);
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_rwlock_rdwait(locker, result);
#endif
  return result;
}
  ...
  )
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    ...
    if (likely(locker != NULL))
      PSI_serveur->start_rwlock_wrwait(locker, src_file, src_line);
  }
#endif
  result= rw_wrlock(&that->m_rwlock);
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_rwlock_wrwait(locker, result);
#endif
  return result;
}
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(PSI_serveur && that->m_psi))
    PSI_serveur->unlock_rwlock(that->m_psi);
#endif
...
{
#ifdef HAVE_PSI_INTERFACE
  that->m_psi= (PSI_serveur ? PSI_serveur->init_cond(key, &that->m_cond)
                           : NULL);
#else
  that->m_psi= NULL;
#endif
  ...
  mysql_cond_t *that)
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    PSI_serveur->destroy_cond(that->m_psi);
    that->m_psi= NULL;
  }
#endif
  ...
  )
{
  ...
  if (likely(PSI_serveur && that->m_psi))
  {
    locker= PSI_serveur->get_thread_cond_locker(&state, that->m_psi, mutex->m_psi,
                                               PSI_COND_WAIT);
    if (likely(locker != NULL))
      PSI_serveur->start_cond_wait(locker, src_file, src_line);
  }
#endif
  result= pthread_cond_wait(&that->m_cond, &mutex->m_mutex);
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_cond_wait(locker, result);
#endif
  return result;
}
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(PSI_serveur && that->m_psi))
    PSI_serveur->signal_cond(that->m_psi);
#endif
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(PSI_serveur && that->m_psi))
    PSI_serveur->broadcast_cond(that->m_psi);
#endif
  ...
  void *(*start_routine)(void*), void *arg)
{
  ...
  if (likely(PSI_serveur != NULL))
    result= PSI_serveur->spawn_thread(key, thread, attr, start_routine, arg);
  else
...
static inline void inline_mysql_thread_set_psi_id(ulong id)
{
  if (likely(PSI_serveur != NULL))
  {
    struct PSI_thread *psi= PSI_serveur->get_thread();
    if (likely(psi != NULL))
      PSI_serveur->set_thread_id(psi, id);
  }
}
#endif
