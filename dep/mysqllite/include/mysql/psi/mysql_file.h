  ...
  char *str, int size, MYSQL_FILE *file)
{
  ...
  if (likely(PSI_serveur && file->m_psi))
  {
    locker= PSI_serveur->get_thread_file_stream_locker(&state, file->m_psi,
                                                      PSI_FILE_READ);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) size, src_file, src_line);
  }
#endif
  result= fgets(str, size, file->m_file);
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_wait(locker, result ? strlen(result) : 0);
#endif
  return result;
}
  ...
  MYSQL_FILE *file)
{
  ...
  if (likely(PSI_serveur && file->m_psi))
  {
    locker= PSI_serveur->get_thread_file_stream_locker(&state, file->m_psi,
                                                      PSI_FILE_READ);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 1, src_file, src_line);
  }
#endif
  result= fgetc(file->m_file);
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_wait(locker, (size_t) 1);
#endif
  return result;
}
  ...
  const char *str, MYSQL_FILE *file)
{
  ...
  if (likely(PSI_serveur && file->m_psi))
  {
    ...
    if (likely(locker != NULL))
    {
      ...
      PSI_serveur->start_file_wait(locker, bytes, src_file, src_line);
    }
  }
#endif
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_wait(locker, bytes);
#endif
  return result;
}
  ...
  */
  ...
  if (likely(PSI_serveur && file->m_psi))
  {
    locker= PSI_serveur->get_thread_file_stream_locker(&state, file->m_psi,
                                                      PSI_FILE_WRITE);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, __FILE__, __LINE__);
  }
#endif
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_wait(locker, (size_t) result);
#endif
  return result;
}
  ...
  MYSQL_FILE *file, const char *format, va_list args)
{
  ...
  if (likely(PSI_serveur && file->m_psi))
  {
    locker= PSI_serveur->get_thread_file_stream_locker(&state, file->m_psi,
                                                      PSI_FILE_WRITE);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
#endif
  result= vfprintf(file->m_file, format, args);
#ifdef HAVE_PSI_INTERFACE
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_wait(locker, (size_t) 0);
#endif
  return result;
}
  ...
  int filenr, MY_STAT *stat_area, myf flags)
{
  ...
  struct PSI_file_locker *locker= NULL;
  PSI_file_locker_state state;
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_descriptor_locker(&state, filenr,
                                                          PSI_FILE_FSTAT);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
  ...
  const char *path, MY_STAT *stat_area, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_name_locker(&state,
                                                    key, PSI_FILE_STAT,
                                                    path, &locker);
    if (likely(locker != NULL))
      PSI_serveur->start_file_open_wait(locker, src_file, src_line);
  }
#endif
  ...
  File file, my_off_t newlength, int filler, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_descriptor_locker(&state, file,
                                                          PSI_FILE_CHSIZE);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) newlength, src_file,
                                  src_line);
  }
#endif
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_wait(locker, (size_t) newlength);
#endif
  return result;
}
  ...
  const char *filename, int flags, myf myFlags)
{
  ...
  if (likely(that != NULL))
  {
    that->m_psi= NULL;
    {
      ...
      if (likely(PSI_serveur != NULL))
      {
        locker= PSI_serveur->get_thread_file_name_locker
          (&state, key, PSI_FILE_STREAM_OPEN, filename, that);
        if (likely(locker != NULL))
          that->m_psi= PSI_serveur->start_file_open_wait(locker, src_file,
                                                        src_line);
      }
#endif
...
#ifdef HAVE_PSI_INTERFACE
      if (likely(locker != NULL))
        PSI_serveur->end_file_open_wait(locker);
#endif
  ...
  MYSQL_FILE *file, uchar *buffer, size_t count, myf flags)
{
  ...
  if (likely(PSI_serveur && file->m_psi))
  {
    locker= PSI_serveur->get_thread_file_stream_locker(&state, file->m_psi,
                                                      PSI_FILE_READ);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, count, src_file, src_line);
  }
#endif
  ...
  if (likely(locker != NULL))
  {
    ...
    PSI_serveur->end_file_wait(locker, bytes_read);
  }
#endif
  return result;
}
  ...
  MYSQL_FILE *file, const uchar *buffer, size_t count, myf flags)
{
  ...
  if (likely(locker != NULL))
  {
    ...
    PSI_serveur->end_file_wait(locker, bytes_written);
  }
#endif
  return result;
}
  ...
  const char *filename, int create_flags, int access_flags, myf myFlags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_name_locker(&state, key, PSI_FILE_CREATE,
                                                    filename, &locker);
    if (likely(locker != NULL))
      PSI_serveur->start_file_open_wait(locker, src_file, src_line);
  }
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(locker != NULL))
    PSI_serveur->end_file_open_wait_and_bind_to_descriptor(locker, file);
#endif
  return file;
}
...
#ifdef HAVE_PSI_INTERFACE
  if (likely(PSI_serveur != NULL))
    PSI_serveur->create_file(key, to, file);
#endif
  return file;
}
  ...
  const char *filename, int flags, myf myFlags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_name_locker(&state, key, PSI_FILE_OPEN,
                                                    filename, &locker);
    if (likely(locker != NULL))
      PSI_serveur->start_file_open_wait(locker, src_file, src_line);
  }
  ...
  File file, uchar *buffer, size_t count, my_off_t offset, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_descriptor_locker(&state, file, PSI_FILE_READ);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, count, src_file, src_line);
  }
#endif
  ...
  File file, my_off_t pos, int whence, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_descriptor_locker(&state, file, PSI_FILE_SEEK);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
#endif
  ...
  File file, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_descriptor_locker(&state, file, PSI_FILE_TELL);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
#endif
  ...
  const char *name, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_name_locker(&state, key, PSI_FILE_DELETE,
                                                    name, &locker);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
  ...
  const char *from, const char *to, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_name_locker(&state, key, PSI_FILE_RENAME,
                                                    to, &locker);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
  ...
  File fd, myf flags)
{
  ...
  if (likely(PSI_serveur != NULL))
  {
    locker= PSI_serveur->get_thread_file_descriptor_locker(&state, fd, PSI_FILE_SYNC);
    if (likely(locker != NULL))
      PSI_serveur->start_file_wait(locker, (size_t) 0, src_file, src_line);
  }
#endif
